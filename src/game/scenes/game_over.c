void displayGameOver(App* app)
{
    ...
    sprintf(output, "Score: %d", );*/
    ImGuiIO* io = igGetIO();
    /*cvAddText((io->DisplaySize.x/50)/2 - ((strlen("GAME OVER")) * 0.3)/2, -(io->DisplaySize.y/50)/2 +1.5, CV_COL32_WHITE, "GAME OVER");
    cvAddText(
        (io->DisplaySize.x/50)/2 - ((strlen(output)) * 0.225)/2 , //Formula to move text when there are more numbers
        -(io->DisplaySize.y/50)/2 +0.5, CV_COL32_WHITE, output);*/
}